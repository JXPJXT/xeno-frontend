 ...
 */
async function resolveSegmentId(segmentName: string, h: Record<string, string>): Promise<string> {
  ...
  try {
    const campaigns = await call(`${API_BASE}/campaigns`, { headers: h });
...
export async function executeTool(
  ...
  switch (name) {
    case 'get_platform_overview':
      return call(`${API_BASE}/analytics/overview`, { headers: h });
    ...
    case 'get_customers': {
      ...
      return call(`${API_BASE}/customers?${params}`, { headers: h });
    }

    case 'get_customer_intelligence':
      return call(`${API_BASE}/intelligence/customers/${args.customerId}`, { headers: h });

    case 'get_campaigns':
      return call(`${API_BASE}/campaigns`, { headers: h });

    case 'get_campaign_results':
      return call(`${API_BASE}/analytics/campaigns/${args.campaignId}`, { headers: h });

    case 'create_campaign': {
      // Resolve segment name to UUID
      let segmentId = args.segmentId;
      if (!segmentId && args.segmentName) {
        segmentId = await resolveSegmentId(args.segmentName, h);
      }
